...
function player_input(){
    for (let i = 0; i <tag_list.length ; i++){
        function change_to_X(){
            if (tag_list[i].textContent == undefined || tag_list[i].textContent == ''){
...
function draw_condition(){
    for (let i = 0; i <tag_list.length ; i++){
        if (tag_list[i].textContext == '' || tag_list[i].textContext == undefined){
            console.log(tag_list[i].textContent);
            return false;
        }
    }
    return true;
